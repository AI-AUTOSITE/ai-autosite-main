...
// Mobile optimized version with responsive design
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 sm:gap-3 text-xs sm:text-sm">
        <span className="px-2 sm:px-3 py-1 bg-green-500/20 text-green-400 rounded-full">
          Confidence: {Math.round(confidence * 100)}%
        </span>
        <span className="px-2 sm:px-3 py-1 bg-blue-500/20 text-blue-400 rounded-full">
          Characters: {text.length}
        </span>
        {detectedLanguage && detectedLanguage !== 'unknown' && (
          <span className="px-2 sm:px-3 py-1 bg-purple-500/20 text-purple-400 rounded-full flex items-center gap-1">
            <Languages className="w-3 h-3" />
            <span className="hidden sm:inline">
              Detected: {detectedLanguage === 'japanese' ? 'Japanese' : 'English'}
            </span>
            <span className="sm:hidden">
              {detectedLanguage === 'japanese' ? 'JA' : 'EN'}
            </span>
          </span>
        )}
        {translationDirection && (
          <span className="px-2 sm:px-3 py-1 bg-cyan-500/20 text-cyan-400 rounded-full flex items-center gap-1">
      ...
      <div className="bg-black/30 rounded-lg p-3 sm:p-4">
        ...
        <pre className="text-gray-200 whitespace-pre-wrap font-mono text-xs sm:text-sm max-h-64 sm:max-h-96 overflow-y-auto">
      ...
      {isTranslating ? (
        <div className="bg-black/30 rounded-lg p-3 sm:p-4">
          ...
          <div className="flex items-center gap-2 text-gray-400">
            ...
            <p className="italic text-sm sm:text-base">Processing translation...</p>
          </div>
        </div>
      ) : translation ? (
        <div className="bg-black/30 rounded-lg p-3 sm:p-4">
          ...
          <p className="text-gray-200 whitespace-pre-wrap text-sm sm:text-base">{translation}</p>
          <p className="text-xs text-gray-400 mt-2">Powered by Translation API</p>
        </div>
      ) : (
        <div className="bg-black/30 rounded-lg p-3 sm:p-4">
          <h3 className="text-white font-semibold mb-2 text-sm sm:text-base">Translation</h3>
          <p className="text-gray-400 italic text-sm sm:text-base">Translation not available</p>
        </div>
      )}
    </div>
  )
