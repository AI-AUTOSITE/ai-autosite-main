// app/tools/loop-audio/components/LoopAudioClient.tsx
'use client';
...
import { loopAudio } from '@/lib/audio-toolkit';
import { 
  Shield, Zap, Play, Pause, RotateCcw, Download, Clock, HardDrive, Repeat,
  ChevronDown, ChevronUp, HelpCircle, ArrowRight, Plus, Minus
} from 'lucide-react';

type OutputFormat = 'wav' | 'mp3';

export default function LoopAudioClient() {
  // State
  ...
  const handleProcessAndDownload = useCallback(async () => {
    ...
    try {
      // ループ適用
      const processedBuffer = loopAudio(buffer, loopCount, crossfade);
      ...
      download(blob, `${baseName}_loop${loopCount}x.${extension}`);
    } catch (err) {
      console.error('Loop processing failed:', err);
    } finally {
      setIsProcessing(false);
    }
  }, [buffer, file, loopCount, crossfade, outputFormat, exportToWav, exportToMp3, download]);
  ...
  // 最大クロスフェード時間（音声の半分まで）
  const maxCrossfade = bufferInfo ? bufferInfo.duration / 2 : 5;
  
  // ループ後の長さ（概算）
  const estimatedDuration = bufferInfo 
    ? (bufferInfo.duration - crossfade) * loopCount + crossfade
    : 0;
  ...
  const faqs = [
    {
      q: 'What is audio looping?',
      a: 'Looping repeats your audio multiple times in sequence. It\'s useful for creating longer versions of short clips, music production, and background audio.',
    },
    {
      q: 'What is crossfade?',
      a: 'Crossfade creates a smooth transition between loop iterations by gradually fading out the end of one loop while fading in the next. This prevents clicks or pops.',
    },
    {
      q: 'How many times can I loop?',
      a: 'You can loop from 2 to 20 times. For longer loops, you may need to process multiple times.',
    },
    {
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-gray-400 max-w-2xl mx-auto">
            Create seamless audio loops. Repeat audio with optional crossfade transitions.
        ...
        <div className="bg-gray-800/50 backdrop-blur-xl rounded-2xl border border-gray-700 p-6 space-y-6">
            ...
            <>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <label className="text-sm text-gray-400 font-medium">Loop Count</label>
                  <span className="text-2xl font-bold text-emerald-400">{loopCount}x</span>
                </div>
                
                <div className="flex items-center gap-4">
                  <button
                    onClick={() => setLoopCount(Math.max(2, loopCount - 1))}
                    disabled={loopCount <= 2 || isWorking}
                  ...
                  <button
                    onClick={() => setLoopCount(Math.min(20, loopCount + 1))}
                    disabled={loopCount >= 20 || isWorking}
                    className="p-2 bg-gray-700 hover:bg-gray-600 rounded-lg transition-colors disabled:opacity-50"
                  >
          ...
          {buffer && !isWorking && (
            ...
            >
              ...
              Create Loop & Download {outputFormat.toUpperCase()}
            </button>
          )}
            ...
            <div className="p-4 border-t border-gray-700 space-y-4">
              <div className="grid gap-4 md:grid-cols-3">
                {[
                  ...
                  { title: 'Set Loop Count', desc: 'Choose how many times to repeat.' },
                  { title: 'Download', desc: 'Click to create loop and download.' },
                ].map((step, i) => (
