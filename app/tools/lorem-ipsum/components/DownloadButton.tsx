    ...
    <div className="relative">
      ...
      {isOpen && !disabled && (
        <>
          ...
          {/* Menu */}
          <div className="absolute right-0 mt-2 w-56 bg-gray-800 border border-white/10 rounded-xl shadow-lg z-20 overflow-hidden">
            <div className="p-2">
              <div className="text-xs text-gray-400 px-3 py-2 font-medium">Download as:</div>
              {FORMATS.map((format) => (
                <button
                  key={format}
                  onClick={() => handleDownload(format)}
                  className="w-full text-left px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-white/10 transition-colors flex items-center justify-between group"
                >
