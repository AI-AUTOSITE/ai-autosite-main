...
export default function ImageCompressClient() {
  ...
  const handleFiles = (files: FileList | null) => {
    ...
    if (currentCount + newCount > MAX_FILES) {
      setErrorMessage(`Maximum ${MAX_FILES} images allowed${isMobile ? ' on mobile' : ''}. Please remove some images first.`)
      return
    }
    ...
    Array.from(files).forEach(file => {
      ...
      if (file.size > MAX_FILE_SIZE) {
        ...
        errors.push(`${file.name}: File too large (max ${maxSizeMB}MB${isMobile ? ' on mobile' : ''})`)
        return
      }
        ...
        {images.length > 0 && (
          <div className="mt-4 p-3 bg-black/20 rounded-lg flex items-center justify-between text-xs">
            ...
            {errorCount > 0 && (
              <button
                onClick={retryFailed}
                className="px-3 py-2 text-red-400 hover:text-red-300 underline text-xs font-medium"
              >
                Try Again
              </button>
            )}
                ...
                >
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-white text-sm sm:text-sm truncate mb-1">
                        {image.name}
                      </p>
                      ...
                      <div className="sm:hidden">
                        {image.status === 'done' && (
                          <div className="flex flex-col">
                            <span className="text-green-400 font-bold text-xl leading-tight">
                              -{Math.round((1 - image.compressedSize / image.originalSize) * 100)}%
                            </span>
                        ...
                        {image.status === 'processing' && (
                          <div className="flex flex-col">
                            <span className="text-cyan-400 text-sm">Working...</span>
                            {image.progress !== undefined && (
                              <span className="text-cyan-400 font-medium text-lg">{image.progress}%</span>
                            )}
                          </div>
                        )}
                        {image.status === 'error' && (
