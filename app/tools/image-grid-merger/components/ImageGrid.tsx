...
const MAX_CELLS = 64 // 8x8 grid

type CellImage = { file: File; url: string } | null
...
export default function ImageGrid() {
  ...
  useEffect(() => {
    ...
    const generatePreview = async () => {
      ...
      const scale = 0.3 // プレビュー用にスケールダウン
  ...
  const merge = async () => {
    ...
    cvs.toBlob((blob) => {
      ...
      a.href = url
      a.download = `grid_${rows}x${cols}_merged.png`
      a.click()
      ...
      <div className="flex flex-wrap items-center gap-4">
        <label className="flex items-center gap-2 text-white">
          ...
          <input
            ...
            className="w-16 px-2 py-1 rounded bg-black/30 border border-white/20 text-white"
          />
        </label>
        <label className="flex items-center gap-2 text-white">
          <span>Columns:</span>
      ...
      {previewUrl && (
        <div className="bg-black/20 rounded-xl p-4 border border-white/10">
          ...
          <p className="text-gray-400 text-sm text-center mt-2">
            This is a preview. Click "Merge & Download" to save full resolution.
          </p>
        </div>
      )}
...
// Dropzone と Cell コンポーネントは変更なし（前回のコードと同じ）
