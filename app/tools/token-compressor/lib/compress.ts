...
export function compressText(
  ...
  const isCode = isJavaScript || isTSX || isPython || /java|cpp|c\+\+|csharp|php|ruby|go|rust|kotlin|swift/.test(fileType)
  ...
  if (options.removeWhitespace) {
    ...
    if (options.level === 'aggressive' && isCode) {
      ...
      compressed = compressed
        ...
        .replace(/(const|let|var|function|class|export|import|return|if|else|for|while|switch|case|break|continue|throw|try|catch|finally|new|typeof|instanceof|in|of)([a-zA-Z0-9_\(\{])/g, '$1 $2')
        // Fix common patterns
        .replace(/\)\{/g, ') {')
        .replace(/\}else/g, '} else')
        .replace(/else\{/g, 'else {')
...
function aggressiveJSCompression(code: string): string {
  ...
  compressed = compressed
    ...
    .replace(/(const|let|var|function|class|export|import|return|if|else|for|while|new|typeof|instanceof)([a-zA-Z(])/g, '$1 $2')
