    ...
    <div className="h-64 bg-gradient-to-br from-slate-800 to-slate-900 rounded-lg p-4 border border-white/10">
      ...
      <div className="mt-8 p-4 bg-white/5 rounded-lg">
        ...
        <p className="text-gray-400 text-sm">
          You are here: Home → Products → Electronics → Laptops
        </p>
      </div>
    </div>
  ),
    ...
    <div className="h-64 bg-gradient-to-br from-slate-800 to-slate-900 rounded-lg p-4 border border-white/10">
      <div className="mb-4 p-4 bg-white/5 rounded-lg">
        ...
        <p className="text-gray-400 text-sm mt-1">
          Showing items {(demoState.pagination - 1) * 10 + 1}-{demoState.pagination * 10}
        </p>
      </div>
      <div className="flex items-center justify-center gap-2">
        <button
          onClick={() =>
            setDemoState({ ...demoState, pagination: Math.max(1, demoState.pagination - 1) })
          }
        ...
        <button
          onClick={() =>
            setDemoState({ ...demoState, pagination: Math.min(5, demoState.pagination + 1) })
          }
          className="px-3 py-1 bg-white/10 text-gray-300 rounded hover:bg-white/20"
        >
          Next
    ...
    <div className="h-64 bg-gradient-to-br from-slate-800 to-slate-900 rounded-lg p-4 border border-white/10">
          ...
          <div key={i} className="flex flex-col items-center">
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center text-sm ${
                i + 1 <= demoState.stepper ? 'bg-cyan-500 text-white' : 'bg-white/10 text-gray-500'
              }`}
            >
              {i + 1}
            </div>
      ...
      <div className="text-center">
        <p className="text-white mb-4">
          Step {demoState.stepper}:{' '}
          {['Details', 'Payment', 'Review', 'Complete'][demoState.stepper - 1]}
        </p>
        <div className="flex justify-center gap-2">
          <button
            onClick={() =>
              setDemoState({ ...demoState, stepper: Math.max(1, demoState.stepper - 1) })
            }
          ...
          <button
            onClick={() =>
              setDemoState({ ...demoState, stepper: Math.min(4, demoState.stepper + 1) })
            }
