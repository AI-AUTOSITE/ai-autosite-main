...
export class SignatureHandler {
  ...
  static async addSignature(file: File, options: SignatureOptions): Promise<Uint8Array> {
  ...
  private static async addTextSignature(page: PDFPage, options: SignatureOptions) {
  ...
  private static async addDateStamp(page: PDFPage, options: SignatureOptions) {
    const font = await page.doc.embedFont(StandardFonts.Helvetica)
    const date = new Date()
  ...
  static async addStamp(file: File, options: StampOptions): Promise<Uint8Array> {
    const arrayBuffer = await file.arrayBuffer()
    const pdfDoc = await PDFDocument.load(arrayBuffer)
    const pages = pdfDoc.getPages()
