...
export async function POST(request: NextRequest) {
  try {
    if (!process.env.ANTHROPIC_API_KEY) {
      return createErrorResponse('API_ERROR', 'Service configuration error', 500)
    }
    ...
    if (file.type !== ACCEPTED_FILE_TYPE) {
      return createErrorResponse('INVALID_FILE_TYPE', 'Invalid file type. Please upload a PDF file')
    }

    const fileName = file.name.toLowerCase()
    if (!fileName.endsWith(ACCEPTED_FILE_EXTENSION)) {
      return createErrorResponse('INVALID_FILE_TYPE', 'File must have .pdf extension')
    }
    ...
    const message = await anthropic.messages.create({
      ...
      temperature: 0.3,
      system:
        'You are a document summarization expert. Create clear, well-structured summaries in Markdown format with proper headings, bullet points, and formatting.',
      messages: [
        {
          role: 'user',
...
- Proper spacing and formatting`,
    ...
    if (!summary) {
      return createErrorResponse('EXTRACTION_FAILED', 'Failed to generate summary', 500)
    }
    ...
    if (error.status === 401) {
      return createErrorResponse('API_ERROR', 'API authentication failed', 500)
    }
    ...
    return createErrorResponse('API_ERROR', 'Failed to process PDF. Please try again.', 500)
  }
}